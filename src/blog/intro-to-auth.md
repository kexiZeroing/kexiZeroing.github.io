...
Access tokens are opaque to the client. They can change at any time. They should have short expiration times, so a user may frequently get new ones. _Refresh tokens are used to obtain a renewed access token without having to re-authenticate the user._ The client application should never contain code that relies on the contents of the access token.

```js
// Refresh token (expire in days) -> HTTP only cookie
// Access token (expire in mins) -> In memory state
...
_(above picture comes from https://auth0.com/blog/id-token-access-token-what-is-the-difference)_
