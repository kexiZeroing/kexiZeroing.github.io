...
单纯的了解协议是不够的，我们需要在应用层使用这些协议和加密算法的软件，比如 Shadowsocks，ShadowsocksR，V2Ray, Clash, Surge 等 _（Shadowsocks 既是协议，也有同名的软件）_。软件方式的明显缺点是没有很高的底层权限，只能代理应用层的网络流量。如果要实现真的全局代理，就要路由器配置科学上网，这样可以无视你的计算机操作系统的代理规则，强制接管所有流量，理论上所有连接到路由器的设备都可以使用代理上网，缺点是大量的加密解密计算会影响路由器的性能。这就引出了软路由，简单说就是改造过的路由器，性能强大，技术门槛较高。网关模式是指通过一台计算机上跑的软件来实现路由器的功能，在本地局域网里部署了一台运行 Clash 的电脑当做网关。
