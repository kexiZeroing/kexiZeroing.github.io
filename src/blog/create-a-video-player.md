...
video.addEventListener("timeupdate", () => {
  ...
  progressBar.style.width = `${
    Math.floor(video.currentTime * 100 / video.duration)
  }%`;
});
```
...
Without any user interaction there is usually only one time range, but if you jump about in the media more than one time range can appear. _The time ranges are ordered by start time in ascending order._
