...
export { draw, name, reportArea, reportPerimeter };
```
...
```javascript
import { draw, name, reportArea, reportPerimeter } from "./modules/square.js";
...
// inside main.js
import { anotherNewFunctionName, newFunctionName } from "./modules/module.js";
...
import { draw, name as squareName } from "./shapes/square.js";
...
[This proposal](https://github.com/WICG/import-maps) allows control over what URLs get fetched by JavaScript `import` statements. This allows "bare import specifiers", such as `import moment from "moment"`, to work. The mechanism for doing this is via an _import map_ which can be used to control the resolution of module specifiers generally.

Today, `import moment from "moment"` throws, as such bare specifiers are reserved _(specifier needs to start with the character `/`, `./`, `../`)_. By supplying the browser with the following import map, the above would act as if you had written `import moment from "/node_modules/moment/src/moment.js"`.
...
_Update at 2023-03-28:_ [JavaScript import maps are now supported cross-browser](https://web.dev/import-maps-in-all-modern-browsers). A modern way to use ES modules is with the `<script type="importmap">` tag. This tag allows you to define a mapping of external module names to their corresponding URLs, which makes it easier to include and use external modules in your code.
