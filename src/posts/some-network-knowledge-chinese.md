...
- Shadowsocks 是一个 project，是一种传输协议（ShadowsocksR, V2Ray, Trojan 也都是协议），分为 client 端和 server 端，实现了两端之间的加密数据传输，它不是一个具体的软件或工具。ShadowsocksX-NG 是 Mac 上免费的一个客户端，不同客户端主要是界面的区别，选择一个使用即可。
